...
<div class="app">
  ...
  <nav>
    <a href="/"><img src="logo.png?v=3" class="nav-logo" alt="Free Text Motion"></a>
  ...
  <aside class="panel panel-left">
    ...
    <div class="sec-title">Texto y Tipografía</div>
    <div class="field">
...
POWER</textarea>
    ...
    <p class="hint">Selecciona una letra para editarla sola. Arrastra para moverla; el tirador naranja cambia su tamaño.</p>
    <div class="field">
  ...
  <aside class="panel panel-right">
    ...
    <div class="sec-title">Animación &mdash; 205 presets</div>
    <div class="field">
      <input type="text" id="presetSearch" placeholder="Buscar efecto...">
    </div>
    <div id="presetList"></div>

    <div class="sec-title">Parámetros del preset</div>
    ...
    <div class="field">
      <label>Duración de entrada</label>
      <div class="field-row">
    ...
    <div class="field">
      <label>Duración de salida</label>
      <div class="field-row">
    ...
    <div class="sec-title">Transformación</div>
    <div class="field">
    ...
    <div class="field">
      <label id="rotationLabel">Rotación</label>
      <div class="field-row">
    ...
    <div id="kfPanel"></div>
    <p class="hint">Activa una propiedad, haz clic en la barra para añadir keyframes en ese instante de la línea de tiempo y arrastra los puntos para moverlos.</p>
...
const KF_PROPS=[
  {key:'size',label:'Tamaño',unit:'px',min:20,max:300,step:1,el:'fontSize'},
  ...
  {key:'rotation',label:'Rotación',unit:'°',min:-180,max:180,step:1,el:'rotation'},
...
const PRESETS=[
  ...
  { id:'typewriter', family:'Máquina', name:'Máquina de escribir', defStagger:0, defInDur:2, fx:C=>({alpha:(C.p*C.totalChars-C.i)>0?1:0}) },
  { id:'decoder', family:'Máquina', name:'Flash letra a letra', defStagger:60, defInDur:2, fx:C=>({alpha:Math.min(1,C.st*3), scX:0.7+0.3*Math.min(1,C.st*2), scY:0.7+0.3*Math.min(1,C.st*2)}) },
  { id:'glitchWriter', family:'Máquina', name:'Escritura glitch', defStagger:25, params:[{key:'speed',label:'Velocidad',min:2,max:20,step:1,def:10}], fx:(C,P)=>{ const g=C.st>0&&C.st<1; const dx=g?(hash(C.i+C.t*P.speed*2)-0.5)*8:0; const dy=g?(hash(C.i+50+C.t*P.speed*2)-0.5)*6:0; return {dx, dy, alpha:Math.min(1,C.st*4)}; } },
  { id:'scramble', family:'Máquina', name:'Mezcla', defStagger:40, params:[{key:'power',label:'Distancia',min:20,max:150,step:5,def:80}], fx:(C,P)=>{ const d=(1-C.st)*P.power; const a=hash(C.i)*Math.PI*2; return {dx:Math.cos(a)*d, dy:Math.sin(a)*d, rot:(1-C.st)*(hash(C.i+9)-0.5)*120, alpha:Math.min(1,C.st*3)}; } },
  { id:'matrixRain', family:'Máquina', name:'Lluvia', defStagger:40, defInDur:2.5, fx:C=>{ const T={dy:(1-C.st)*-90, alpha:Math.min(1,C.st*3)}; if(C.st<1)T.dupes=[{dy:18,alpha:0.3},{dy:36,alpha:0.12}]; return T; } },
  ...
  { id:'pendulum', family:'Ondas', name:'Péndulo', defStagger:25, params:[{key:'amp',label:'Amplitud',min:5,max:45,step:1,def:20},{key:'speed',label:'Velocidad',min:1,max:12,step:1,def:4}], fx:(C,P)=>({rot:Math.sin(C.t*P.speed+C.i*0.4)*P.amp*Math.min(1,C.e*2), alpha:Math.min(1,C.e*2)}) },
  ...
  { id:'stomp', family:'Rebote', name:'Pisotón', defStagger:50, fx:C=>({scY:1-Math.sin(C.e*Math.PI)*0.45, scX:1+Math.sin(C.e*Math.PI)*0.45, dy:Math.sin(C.e*Math.PI)*C.state.size*0.25, alpha:1}) },
  { id:'elasticDrop', family:'Rebote', name:'Caída elástica', defStagger:40, defEasing:'elastic', params:[{key:'amp',label:'Altura',min:50,max:250,step:10,def:160}], fx:(C,P)=>({dy:(1-C.e)*-P.amp, alpha:Math.min(1,C.e*2)}) },
  ...
  { id:'rainbowFlow', family:'Color', name:'Flujo arcoíris', defStagger:15, params:[{key:'speed',label:'Velocidad',min:1,max:20,step:1,def:4}], fx:(C,P)=>({fill:'hsl('+((C.t*P.speed*20+C.i*12)%360)+',85%,55%)', alpha:Math.min(1,C.e*3)}) },
  { id:'neonPulse', family:'Color', name:'Pulso neón', defStagger:0, params:[{key:'speed',label:'Velocidad',min:1,max:10,step:1,def:4}], fx:(C,P)=>({fill:C.state.fill, shadow:C.state.stroke, shadowBlur:10+Math.sin(C.t*P.speed)*8, alpha:Math.min(1,C.e*2)}) },
  ...
  { id:'explode', family:'Salida y ambient', name:'Explosión', defStagger:0, params:[{key:'power',label:'Potencia',min:50,max:400,step:10,def:220}], fx:(C,P)=>{ const a=hash(C.i)*Math.PI*2; const d=(1-C.e)*P.power; return {dx:Math.cos(a)*d, dy:Math.sin(a)*d, rot:(1-C.e)*hash(C.i+9)*180, alpha:Math.min(1,C.e*2)}; } },
  { id:'gravityFall', family:'Salida y ambient', name:'Caída libre', defStagger:30, params:[{key:'power',label:'Altura',min:80,max:400,step:10,def:200}], fx:(C,P)=>({dy:(1-C.e)*-P.power+Math.abs(Math.sin(C.e*Math.PI*2))*P.power*0.5, alpha:Math.min(1,C.e*2)}) },
  { id:'scatter', family:'Salida y ambient', name:'Dispersión', defStagger:0, params:[{key:'power',label:'Alcance',min:100,max:400,step:10,def:250}], fx:(C,P)=>({dx:(hash(C.i*5)-0.5)*2*P.power*(1-C.e), dy:(hash(C.i*11)-0.5)*2*P.power*(1-C.e), rot:(hash(C.i*7)-0.5)*90*(1-C.e), alpha:Math.min(1,C.e*2)}) },
  ...
  { id:'rubberBand', family:'Elásticos', name:'Banda elástica', defStagger:35, fx:C=>({scX:1+Math.sin(C.st*Math.PI*2)*0.35*(1-C.st*0.6), scY:1-Math.sin(C.st*Math.PI*2)*0.35*(1-C.st*0.6), alpha:Math.min(1,C.st*3)}) },
  { id:'heartBeat', family:'Elásticos', name:'Latido', defStagger:45, fx:C=>({scX:1+Math.abs(Math.sin(C.st*Math.PI*3))*0.3*(1-C.st*0.4), scY:1+Math.abs(Math.sin(C.st*Math.PI*3))*0.3*(1-C.st*0.4), alpha:Math.min(1,C.st*3)}) },
  { id:'dizzy', family:'Elásticos', name:'Mareo', defStagger:30, fx:C=>({rot:(1-C.st)*220*(hash(C.i)>0.5?1:-1), blur:(1-C.st)*10, alpha:Math.min(1,C.st*3)}) },
  { id:'sneeze', family:'Elásticos', name:'Estornudo', defStagger:50, fx:C=>({scX:1+Math.sin(C.st*Math.PI*2)*0.5, scY:1+Math.sin(C.st*Math.PI*2)*0.5, blur:(1-C.st)*14, alpha:Math.min(1,C.st*4)}) },
  { id:'tickTock', family:'Elásticos', name:'Tic-tac', defStagger:40, fx:C=>({rot:Math.sin(C.st*Math.PI*4)*25*(1-C.st*0.7), alpha:Math.min(1,C.st*3)}) },

  { id:'meltDown', family:'Líquido', name:'Derretido', defStagger:25, fx:C=>({dy:(1-C.st)*-40, scY:1+(1-C.st)*1.2, blur:(1-C.st)*8, alpha:Math.min(1,C.st*2)}) },
  { id:'waveSkew', family:'Líquido', name:'Onda sesgada', defStagger:0, params:[{key:'amp',label:'Amplitud',min:5,max:50,step:5,def:20},{key:'speed',label:'Velocidad',min:1,max:15,step:1,def:5}], fx:(C,P)=>({skew:Math.sin(C.t*P.speed+C.i*0.5)*P.amp/100*Math.min(1,C.e*2), alpha:Math.min(1,C.e*2)}) },
  { id:'splash', family:'Líquido', name:'Salpicadura', defStagger:35, fx:C=>({dy:(1-C.st)*-120+Math.abs(Math.sin(C.st*Math.PI))*40, scX:1+Math.sin(C.st*Math.PI)*0.3, scY:1-Math.sin(C.st*Math.PI)*0.3, alpha:Math.min(1,C.st*2)}) },
  { id:'jellySkew', family:'Líquido', name:'Gelatina sesgada', defStagger:0, params:[{key:'amp',label:'Amplitud',min:5,max:40,step:5,def:20},{key:'speed',label:'Velocidad',min:2,max:12,step:1,def:5}], fx:(C,P)=>({skew:Math.sin(C.t*P.speed+C.i)*P.amp/100*Math.min(1,C.e*2), alpha:Math.min(1,C.e*2)}) },
  { id:'liquidDrop', family:'Líquido', name:'Gota', defStagger:30, params:[{key:'power',label:'Altura',min:40,max:200,step:10,def:100}], fx:(C,P)=>({dy:(1-C.st)*-P.power+Math.abs(Math.sin(C.st*Math.PI))*P.power*0.35, scY:1+(1-C.st)*0.8, alpha:Math.min(1,C.st*2)}) },
  ...
  { id:'loopLoop', family:'Caminos', name:'Rizo', defStagger:45, params:[{key:'power',label:'Tamaño',min:20,max:120,step:10,def:70}], fx:(C,P)=>({dy:-Math.sin(C.st*Math.PI*2)*P.power, dx:-Math.cos(C.st*Math.PI*2)*P.power*0.3+P.power*0.3, alpha:Math.min(1,C.st*2)}) },
  ...
  { id:'pingPong', family:'Física', name:'Ping-pong', defStagger:0, defEasing:'linear', params:[{key:'amp',label:'Recorrido',min:20,max:120,step:10,def:80},{key:'speed',label:'Velocidad',min:1,max:12,step:1,def:3}], fx:(C,P)=>({dx:Math.abs(((C.t*P.speed*2+C.i*0.8)%2)-1)*P.amp-P.amp/2, alpha:Math.min(1,C.e*2)}) },
  { id:'springWobble', family:'Física', name:'Muelle', defStagger:0, params:[{key:'amp',label:'Amplitud',min:5,max:40,step:1,def:18},{key:'speed',label:'Velocidad',min:2,max:15,step:1,def:5}], fx:(C,P)=>({rot:Math.sin(C.t*P.speed+C.i*0.7)*P.amp*Math.min(1,C.e*2), alpha:Math.min(1,C.e*2)}) },
  { id:'ricochet', family:'Física', name:'Ricochet', defStagger:30, params:[{key:'power',label:'Potencia',min:40,max:220,step:10,def:140}], fx:(C,P)=>{ const dir=hash(C.i)>0.5?1:-1; return {dx:dir*(1-C.st)*P.power*0.5, dy:(1-C.st)*-P.power+Math.abs(Math.sin(C.st*Math.PI*3))*P.power*0.7, alpha:Math.min(1,C.st*2)}; } },
  { id:'pendulumSwing', family:'Física', name:'Péndulo de cuerda', defStagger:0, params:[{key:'amp',label:'Amplitud',min:10,max:80,step:5,def:40},{key:'speed',label:'Velocidad',min:1,max:10,step:1,def:3}], fx:(C,P)=>{ const a=Math.sin(C.t*P.speed+C.i*0.5)*P.amp; return {dx:a, dy:-(1-Math.cos(C.t*P.speed+C.i*0.5))*P.amp*0.5, alpha:Math.min(1,C.e*2)}; } },
  { id:'floatAmbient', family:'Física', name:'Flotación', defStagger:0, defInDur:0.8, params:[{key:'speed',label:'Velocidad',min:1,max:10,step:1,def:3}], fx:(C,P)=>({dy:Math.sin(C.t*P.speed+C.i*0.8)*12, rot:Math.sin(C.t*P.speed*0.7+C.i)*2, alpha:Math.min(1,C.e*2)}) },

  { id:'extrudeIn', family:'Profundidad', name:'Extrusión', defStagger:0, params:[{key:'depth',label:'Profundidad',min:2,max:10,step:1,def:5}], fx:(C,P)=>{ const dupes=[]; const shades=['#d9d9d9','#b3b3b3','#8c8c8c','#666666','#404040']; for(let k=1;k<=P.depth;k++)dupes.push({dx:-k*7*(1-C.e), color:shades[(k-1)%5], alpha:0.85}); return {dupes, alpha:C.e}; } },
  ...
  { id:'tunnelIn', family:'Profundidad', name:'Túnel', defStagger:20, fx:C=>({scX:0.2+0.8*C.st, scY:0.2+0.8*C.st, blur:(1-C.st)*15, dy:(1-C.st)*20, alpha:Math.min(1,C.st*2)}) },
  ...
  { id:'baselineBounce', family:'Tipográficos', name:'Saltos de línea base', defStagger:0, defInDur:0.8, params:[{key:'speed',label:'Velocidad',min:2,max:15,step:1,def:6}], fx:(C,P)=>({dy:-Math.abs(Math.sin(C.t*P.speed+C.i*0.6))*14, rot:Math.sin(C.t*P.speed+C.i*0.6)*6, alpha:Math.min(1,C.e*2)}) },
  { id:'swashIn', family:'Tipográficos', name:'Trazo caligráfico', defStagger:35, fx:C=>({rot:(1-C.st)*120*(C.i%2?-1:1), blur:(1-C.st)*8, alpha:Math.min(1,C.st*2)}) },
  { id:'kernDance', family:'Tipográficos', name:'Baile de kerning', defStagger:0, defInDur:0.8, params:[{key:'speed',label:'Velocidad',min:2,max:15,step:1,def:5}], fx:(C,P)=>({kern:Math.sin(C.t*P.speed+C.i*0.4)*24, alpha:Math.min(1,C.e*2)}) },
  { id:'tallIn', family:'Tipográficos', name:'Crecimiento alto', defStagger:30, fx:C=>({scY:1.8-0.8*C.st, scX:0.8+0.2*C.st, alpha:C.st}) },
  { id:'squishIn', family:'Tipográficos', name:'Aplastamiento', defStagger:30, fx:C=>({scY:0.4+0.6*C.st, scX:1.4-0.4*C.st, dy:(1-C.st)*30, alpha:Math.min(1,C.st*2)}) },

  { id:'orbitLoop', family:'Ambientales', name:'Órbita', defStagger:0, defInDur:0.8, params:[{key:'speed',label:'Velocidad',min:1,max:10,step:1,def:3}], fx:(C,P)=>({dx:Math.cos(C.t*P.speed+C.i*0.7)*30, dy:Math.sin(C.t*P.speed+C.i*0.7)*12, alpha:Math.min(1,C.e*2)}) },
  { id:'breathing', family:'Ambientales', name:'Respiración', defStagger:0, defInDur:0.8, params:[{key:'speed',label:'Velocidad',min:1,max:8,step:1,def:2}], fx:(C,P)=>({scX:1+Math.sin(C.t*P.speed+C.i*0.3)*0.08, scY:1+Math.sin(C.t*P.speed+C.i*0.3)*0.08, alpha:1}) },
  ...
  { id:'helicopterLand', family:'Aterrizajes', name:'Helicóptero', defStagger:30, fx:C=>({dy:(1-C.st)*-160, rot:Math.sin(C.st*Math.PI*4)*8*(1-C.st), alpha:Math.min(1,C.st*3)}) },
  ...
  { id:'parachute', family:'Aterrizajes', name:'Paracaídas', defStagger:35, fx:C=>({dy:(1-C.st)*-120, scX:1+(1-C.st)*0.5, scY:1+(1-C.st)*0.5, rot:Math.sin(C.t*3+C.i)*(1-C.st)*6, alpha:Math.min(1,C.st*2)}) },
  ...
  { id:'cineFade', family:'Cinematográficos', name:'Fundido cinematográfico', defStagger:25, defEasing:'expoOut', fx:C=>({alpha:C.e, dy:(1-C.e)*30, blur:(1-C.e)*4}) },
  { id:'cineTrack', family:'Cinematográficos', name:'Tracking cinematográfico', defStagger:20, defEasing:'expoOut', fx:C=>({kern:(1-C.e)*70, alpha:C.e, blur:(1-C.e)*6}) },
  { id:'cineZoom', family:'Cinematográficos', name:'Zoom cinematográfico', defStagger:30, defEasing:'expoOut', fx:C=>({scX:0.85+0.15*C.e, scY:0.85+0.15*C.e, alpha:C.e, blur:(1-C.e)*8}) },
  { id:'whipPan', family:'Cinematográficos', name:'Whip pan', defStagger:15, defEasing:'expoOut', fx:C=>({dx:(1-C.e)*600, rot:(1-C.e)*6, alpha:C.e, blur:(1-C.e)*5}) },
  { id:'glideIn', family:'Cinematográficos', name:'Deslizamiento suave', defStagger:30, fx:C=>({dx:(1-C.e)*-150, alpha:Math.min(1,C.e*1.5)}) },
  { id:'focusPull', family:'Cinematográficos', name:'Tiro de foco', defStagger:20, fx:C=>({blur:(1-C.e)*24, alpha:Math.min(1,C.e*1.6)}) },
  { id:'driftUp', family:'Cinematográficos', name:'Deriva ascendente', defStagger:35, fx:C=>({dy:(1-C.e)*60, alpha:C.e, rot:(1-C.e)*2}) },
  { id:'chapterTitle', family:'Cinematográficos', name:'Título de capítulo', defStagger:25, fx:C=>({scX:0.94+0.06*C.e, scY:0.94+0.06*C.e, dy:(1-C.e)*20, blur:(1-C.e)*3, alpha:C.e}) },
  { id:'microReveal', family:'Cinematográficos', name:'Micro revelado', defStagger:30, fx:C=>({scX:0.98+0.02*C.e, scY:0.98+0.02*C.e, dy:(1-C.e)*8, alpha:C.e}) },
  { id:'softPop', family:'Cinematográficos', name:'Pop suave', defStagger:40, defEasing:'backOut', fx:C=>({scX:0.9+0.1*C.e, scY:0.9+0.1*C.e, alpha:C.e}) },

  { id:'kinFlip', family:'Kinetic Type', name:'Volteo kinético', defStagger:35, fx:C=>({scX:Math.cos((1-C.st)*Math.PI), alpha:1}) },
  { id:'kinJump', family:'Kinetic Type', name:'Salto kinético', defStagger:45, fx:C=>({dy:-Math.abs(Math.sin(C.st*Math.PI))*60, rot:Math.sin(C.st*Math.PI)*14, alpha:1}) },
  { id:'kinSpin', family:'Kinetic Type', name:'Giro kinético', defStagger:30, fx:C=>({rot:(1-C.st)*360, scX:0.4+0.6*C.st, scY:0.4+0.6*C.st, alpha:Math.min(1,C.st*2)}) },
  ...
  { id:'kinPop', family:'Kinetic Type', name:'Pop kinético', defStagger:50, fx:C=>({scX:1+Math.sin(C.st*Math.PI)*0.4, scY:1+Math.sin(C.st*Math.PI)*0.4, alpha:Math.min(1,C.st*2)}) },
  { id:'kinWave', family:'Kinetic Type', name:'Onda kinético', defStagger:25, fx:C=>({dy:Math.sin(C.st*Math.PI*2+C.i*0.8)*30*(1-C.st*0.5), alpha:Math.min(1,C.st*2)}) },
  { id:'kinPunch', family:'Kinetic Type', name:'Golpe kinético', defStagger:35, fx:C=>({dx:(1-C.st)*(hash(C.i)>0.5?1:-1)*90, scX:1.3-0.3*C.st, scY:1.3-0.3*C.st, alpha:Math.min(1,C.st*2)}) },
  { id:'kinRoll', family:'Kinetic Type', name:'Rodado kinético', defStagger:30, fx:C=>({rot:(1-C.st)*180, dy:(1-C.st)*40, alpha:Math.min(1,C.st*2)}) },
  { id:'kinStretch', family:'Kinetic Type', name:'Estiramiento kinético', defStagger:30, fx:C=>({scX:1.6-0.6*C.st, scY:0.5+0.5*C.st, alpha:Math.min(1,C.st*2)}) },
  { id:'kinZoom', family:'Kinetic Type', name:'Zoom kinético', defStagger:30, fx:C=>({scX:0.3+0.7*C.st, scY:0.3+0.7*C.st, rot:(1-C.st)*30, alpha:Math.min(1,C.st*1.5)}) },
  ...
  { id:'edBaseline', family:'Editorial Elegante', name:'Ascenso de línea base', defStagger:30, fx:C=>({dy:(1-C.e)*24, alpha:C.e, blur:(1-C.e)*2}) },
  { id:'edOptical', family:'Editorial Elegante', name:'Ajuste óptico', defStagger:25, fx:C=>({scX:1+(1-C.e)*0.08, scY:1-(1-C.e)*0.05, alpha:C.e}) },
  ...
  { id:'snapIn', family:'Whip y Overshoot', name:'Ajuste elástico', defStagger:25, defEasing:'elastic', fx:C=>({dx:(1-C.e)*-240, alpha:Math.min(1,C.e*2)}) },
  { id:'punchScale', family:'Whip y Overshoot', name:'Escala con puño', defStagger:40, fx:C=>({scX:1+Math.sin(C.e*Math.PI)*0.25, scY:1+Math.sin(C.e*Math.PI)*0.25, alpha:Math.min(1,C.e*2)}) },
  ...
  { id:'stencilIn', family:'Texturas y Grano', name:'Esténcil', forceMode:'stroke', defStagger:25, fx:C=>({strokeW:(1-C.e)*14+2, alpha:C.e}) },
  ...
  { id:'fluidWave', family:'Flujo y Líquido', name:'Onda fluida', defStagger:0, params:[{key:'amp',label:'Amplitud',min:5,max:80,step:1,def:30},{key:'speed',label:'Velocidad',min:1,max:15,step:1,def:5}], fx:(C,P)=>({dy:Math.sin(C.t*P.speed+C.i*0.4)*P.amp*Math.min(1,C.e*2), skew:Math.cos(C.t*P.speed+C.i*0.4)*0.15*Math.min(1,C.e*2), alpha:Math.min(1,C.e*2)}) },
  { id:'ripple2', family:'Flujo y Líquido', name:'Ripple suave', defStagger:0, params:[{key:'amp',label:'Amplitud',min:5,max:40,step:1,def:20},{key:'speed',label:'Velocidad',min:1,max:12,step:1,def:4}], fx:(C,P)=>({scX:1+Math.sin(C.t*P.speed-C.i*0.6)*P.amp/100, scY:1-Math.sin(C.t*P.speed-C.i*0.6)*P.amp/100, alpha:Math.min(1,C.e*2)}) },
  { id:'droplet', family:'Flujo y Líquido', name:'Gota pequeña', defStagger:45, fx:C=>({dy:(1-C.st)*-70+Math.abs(Math.sin(C.st*Math.PI*2))*30, alpha:Math.min(1,C.st*2)}) },
  { id:'jellyWave2', family:'Flujo y Líquido', name:'Gelatina ondulada', defStagger:0, params:[{key:'speed',label:'Velocidad',min:2,max:12,step:1,def:5}], fx:(C,P)=>({scX:1+Math.sin(C.t*P.speed+C.i*0.6)*0.09, scY:1-Math.sin(C.t*P.speed+C.i*0.6)*0.09, alpha:Math.min(1,C.e*2)}) },
  { id:'cascade', family:'Flujo y Líquido', name:'Cascada', defStagger:30, fx:C=>({dy:(1-C.st)*-(40+(C.i%5)*16), alpha:Math.min(1,C.st*2)}) },
  { id:'surf', family:'Flujo y Líquido', name:'Surf', defStagger:0, params:[{key:'speed',label:'Velocidad',min:2,max:12,step:1,def:5}], fx:(C,P)=>({dy:Math.sin(C.t*P.speed+C.i)*14, rot:Math.sin(C.t*P.speed+C.i)*8, alpha:Math.min(1,C.e*2)}) },
  { id:'undulate', family:'Flujo y Líquido', name:'Ondulación', defStagger:0, params:[{key:'speed',label:'Velocidad',min:1,max:12,step:1,def:4}], fx:(C,P)=>({skew:Math.sin(C.t*P.speed+C.i*0.5)*0.3, dy:Math.sin(C.t*P.speed*0.8+C.i)*10, alpha:Math.min(1,C.e*2)}) },
  { id:'bubbleIn', family:'Flujo y Líquido', name:'Burbujas', defStagger:40, defEasing:'bounce', fx:C=>({scX:0.2+0.8*C.st, scY:0.2+0.8*C.st, dy:(1-C.st)*-60, alpha:Math.min(1,C.st*2)}) },
  { id:'splash2', family:'Flujo y Líquido', name:'Salpicadura doble', defStagger:35, fx:C=>({dy:-Math.abs(Math.sin(C.st*Math.PI*2))*90, scX:1+Math.sin(C.st*Math.PI)*0.35, scY:1-Math.sin(C.st*Math.PI)*0.35, alpha:Math.min(1,C.st*2)}) },
  { id:'current', family:'Flujo y Líquido', name:'Corriente', defStagger:30, fx:C=>({dx:(1-C.st)*140, dy:Math.sin(C.t*6+C.i)*8*(1-C.st), alpha:Math.min(1,C.st*2)}) },

  { id:'maskLeft', family:'Minimal Moderno', name:'Máscara izquierda', defStagger:0, defInDur:1.6, fx:C=>({clip:C.st, clipDir:'lr', alpha:1}) },
  { id:'maskRight', family:'Minimal Moderno', name:'Máscara derecha', defStagger:0, defInDur:1.6, fx:C=>({clip:C.st, clipDir:'rl', alpha:1}) },
  { id:'maskTop', family:'Minimal Moderno', name:'Máscara superior', defStagger:0, defInDur:1.6, fx:C=>({clip:C.st, clipDir:'tb', alpha:1}) },
  { id:'maskBottom', family:'Minimal Moderno', name:'Máscara inferior', defStagger:0, defInDur:1.6, fx:C=>({clip:C.st, clipDir:'bt', alpha:1}) },
  ...
  { id:'tiltIn', family:'3D Profundo', name:'Inclinación 3D', defStagger:25, fx:C=>({rot:(1-C.e)*-35, scY:0.8+0.2*C.e, blur:(1-C.e)*5, alpha:C.e}) },
  { id:'rotateY3D', family:'3D Profundo', name:'Rotación Y', defStagger:30, fx:C=>({scX:Math.cos((1-C.e)*Math.PI), alpha:1}) },
  ...
  { id:'orbit3D', family:'3D Profundo', name:'Órbita 3D', defStagger:0, params:[{key:'speed',label:'Velocidad',min:1,max:10,step:1,def:3}], fx:(C,P)=>({scX:Math.cos(C.t*P.speed+C.i*0.5), rot:Math.sin(C.t*P.speed+C.i*0.5)*10, alpha:Math.min(1,C.e*2)}) },
  { id:'flyThrough', family:'3D Profundo', name:'Vuelo a través', defStagger:25, fx:C=>({scX:0.1+0.9*C.st, scY:0.1+0.9*C.st, blur:(1-C.st)*20, alpha:Math.min(1,C.st*2)}) },
  ...
  { id:'letterFall', family:'Letras Objeto', name:'Caída de letras', defStagger:35, fx:C=>({dy:(1-C.st)*-90, alpha:Math.min(1,C.st*2)}) },
  { id:'letterRise', family:'Letras Objeto', name:'Ascenso de letras', defStagger:35, fx:C=>({dy:(1-C.st)*40, alpha:C.st}) },
  { id:'letterShuffle', family:'Letras Objeto', name:'Barajado', defStagger:40, params:[{key:'power',label:'Dispersión',min:20,max:120,step:10,def:60}], fx:(C,P)=>({dx:(hash(C.i)-0.5)*2*P.power*(1-C.st), dy:(hash(C.i+4)-0.5)*2*P.power*(1-C.st), alpha:Math.min(1,C.st*2)}) },
  { id:'letterWobbleIn', family:'Letras Objeto', name:'Vaivén de entrada', defStagger:30, fx:C=>({rot:(1-C.st)*30*Math.sin(C.i), alpha:Math.min(1,C.st*2)}) },
  ...
  { id:'explosion2', family:'Espectaculares', name:'Explosión doble', defStagger:0, params:[{key:'power',label:'Potencia',min:50,max:400,step:10,def:240}], fx:(C,P)=>{ const a=hash(C.i)*Math.PI*2; const d=(1-C.e)*P.power; return {dx:Math.cos(a)*d, dy:Math.sin(a)*d, rot:(1-C.e)*hash(C.i+9)*180, dupes:[{dx:Math.cos(a)*d*0.6,dy:Math.sin(a)*d*0.6,color:'#ffffff',alpha:0.4,gco:'screen'}], alpha:Math.min(1,C.e*2)}; } },
  ...
  { id:'neonFlicker', family:'Espectaculares', name:'Neón parpadeante', defStagger:0, params:[{key:'speed',label:'Velocidad',min:2,max:20,step:1,def:10}], fx:(C,P)=>({shadow:C.state.stroke, shadowBlur:20*(Math.sin(C.t*P.speed+C.i)>0?1:0.3), alpha:1}) },
  ...
  { id:'electric', family:'Espectaculares', name:'Eléctrico', defStagger:0, params:[{key:'speed',label:'Velocidad',min:5,max:30,step:1,def:18}], fx:(C,P)=>({dx:(hash(C.i+Math.floor(C.t*P.speed))-0.5)*10*(1-C.e+0.3), dupes:[{dx:-4,color:'#00e5ff',alpha:0.5,gco:'screen'}], alpha:1}) },
  ...
  { id:'laserIn', family:'Espectaculares', name:'Láser', defStagger:25, fx:C=>({dx:(1-C.e)*300, blur:(1-C.e)*6, dupes:[{dx:-(1-C.e)*40,color:C.state.stroke,alpha:0.6,gco:'screen'}], alpha:C.e}) }
];
const PRESET_BY_ID={};
...
function buildParamsUI(){
  ...
  if(!preset||!preset.params||!preset.params.length){
    wrap.innerHTML='<p class="hint">Este preset no tiene parametros extra. Usa Timing, Transformación y Efectos para personalizarlo.</p>';
    return;
  }
...
function syncStaticSliders(){
  KF_PROPS.forEach(p=>{
    ...
    if(lab){
      ...
      lab.innerHTML=on?p.label+' <span class="kf-on" title="Keyframes activos: este slider edita el keyframe más cercano al cursor. Haz clic aquí para desactivar los keyframes.">keyframes</span>':p.label;
    }
  });
}
...
$('btnDoExport').addEventListener('click',async ()=>{
  ...
  try{
    ...
    alert('Fallo la exportación: '+err.message);
    $('exportMsg').textContent='';
  }
